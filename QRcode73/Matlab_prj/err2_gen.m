function err2_gen(code_m,code_p)
% Á½¸ö´íÎóÉú³É
gen_length = 5000;
code_length = 73;
m_length = 37;
...
for i = 1:gen_length
    ...
    if(err_pos2(i,1) >= m_length)
        ...
        recv_m(i,1) = bitxor(err_m2,temp_m);
        recv_p(i,1) = temp_p;
    else
        err_p2 = bitshift(1,err_pos2(i,1) - 1);
        recv_m(i,1) = temp_m;
        recv_p(i,1) = bitxor(err_p2,temp_p);
    end
end
...
fp = fopen('err2.txt','wt');
fprintf(fp, '_int64 recv_m2[%d] = {\n',gen_length);
for i = 1:gen_length
    if (i < gen_length)
