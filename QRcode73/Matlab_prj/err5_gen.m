function err5_gen(code_m,code_p)
% Îå¸ö´íÎóÉú³É
gen_length = 5000;
code_length = 73;
m_length = 37;
...
for i = 1:gen_length
    ...
    if(err_pos5(i,1) >= m_length)
        ...
        recv_m(i,1) = bitxor(err_m5,temp_m);
        recv_p(i,1) = temp_p;
    else
        err_p5 = bitshift(1,err_pos5(i,1) - 1);
        recv_m(i,1) = temp_m;
        recv_p(i,1) = bitxor(err_p5,temp_p);
    end
end
...
fp = fopen('err5.txt','wt');
fprintf(fp, '_int64 recv_m5[%d] = {\n',gen_length);
for i = 1:gen_length
    if (i < gen_length)
