function err4_gen(code_m,code_p)
% ËÄ¸ö´íÎóÉú³É
gen_length = 5000;
code_length = 73;
m_length = 37;
...
for i = 1:gen_length
    ...
    if(err_pos4(i,1) >= m_length)
        ...
        recv_m(i,1) = bitxor(err_m4,temp_m);
        recv_p(i,1) = temp_p;
    else
        err_p4 = bitshift(1,err_pos4(i,1) - 1);
        recv_m(i,1) = temp_m;
        recv_p(i,1) = bitxor(err_p4,temp_p);
    end
end
...
fp = fopen('err4.txt','wt');
fprintf(fp, '_int64 recv_m4[%d] = {\n',gen_length);
for i = 1:gen_length
    if (i < gen_length)
