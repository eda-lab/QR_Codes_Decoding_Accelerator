...
void main() {
	...
	while (1) {
		...
		if (w_pre <= 6) {
			...
			if (flag == 0) {
				rcv_m_inv = ((rcv_m & shift_1) << 36) ^ rcv_p;
				rcv_p_inv = rcv_m >> 1;
				printf(" rcv_m_inv = %I64d   rcv_p_inv = %I64d\n", rcv_m_inv, rcv_p_inv);
				sym_pre_inv = mod(rcv_m_inv,m_length) ^ rcv_p_inv;
				w_pre_inv = weight(sym_pre_inv,m_length);
				printf(" sym_pre_inv = %I64d  w_pre_inv = %I64d\n", sym_pre_inv, w_pre_inv);
				if (w_pre_inv <= 6) {
					dcode_m = ((rcv_p_inv ^ sym_pre_inv) << 1) ^ (rcv_m_inv >> 36);
					strcpy_s(dcode_pos, "dcode_pre_inv");
					flag = 1;
				}
				else {
					for (i = 0; i < m_length; i++) {				
						w_one_inv = weight(sym_pre_inv ^ sym_table[i], m_length);
						if (w_one_inv <= 5) {
							dcode_m = ((rcv_p_inv ^ sym_pre_inv ^ sym_table[i]) << 1) ^ ((rcv_m_inv ^ (shift_1 << i)) >> 36);
							strcpy_s(dcode_pos, "dcode_one_inv");
							printf("i = %d \n", i);
							flag = 1; break;
						}
					...
					if (flag == 0) {
						for (i = 0; i < m_length; i++) {
							sym_two_inv = sym_pre_inv ^ sym_table[i];
							for (j = i + 1; j < m_length; j++) {
								w_two_inv = weight(sym_two_inv ^ sym_table[j], m_length);
								if (w_two_inv <= 4) {
									dcode_m = ((rcv_p_inv ^ sym_two_inv ^ sym_table[j]) << 1) ^ ((rcv_m_inv ^ (shift_1 << i) ^ (shift_1 << j)) >> 36);
									strcpy_s(dcode_pos, "dcode_two_inv");
									printf("i = %d , j = %d \n", i, j);
									flag = 1; break;
								}
					...
					if (flag == 0) {
						for (i = 0; i < m_length; i++) {
							sym_three1_inv = sym_pre_inv ^ sym_table[i];
							for (j = i + 1; j < m_length; j++) {
								sym_three2_inv = sym_three1_inv ^ sym_table[j];
								for (k = j + 1; k < m_length; k++) {
									w_three_inv = weight(sym_three2_inv ^ sym_table[k], m_length);
									if (w_three_inv <= 3) {
										dcode_m = ((rcv_p_inv ^ w_three_inv ^ sym_table[k]) << 1) ^ ((rcv_m_inv ^ (shift_1 << i) ^ (shift_1 << j) ^ (shift_1 << k)) >> 36);
										strcpy_s(dcode_pos, "dcode_three_inv");
										printf("i = %d , j = %d , k = %d \n", i, j, k);
										flag = 1; break;
									}
